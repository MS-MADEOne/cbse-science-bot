...
# --- STYLING (Bento Grids & Flashcards) ---
st.markdown("""
    <style>
...
def init_ai_discovery():
    try:
        if "GEMINI_KEY" not in st.secrets:
            st.error("Add GEMINI_KEY to Secrets first!")
            return None
            
        genai.configure(api_key=st.secrets["GEMINI_KEY"])
        
        # ASK GOOGLE WHAT MODELS WE HAVE
...
with chat_tab:
    ...
    if p := st.chat_input("Ask a concept..."):
        ...
        with st.chat_message("assistant"):
            if ai_model:
                try:
                    prompt = f"Using context: {raw_text[:6000]}. Explain {p} like a CBSE teacher."
                    res = ai_model.generate_content(prompt).text
                    st.markdown(res)
                    st.session_state.msgs.append({"role": "assistant", "content": res})
                except: st.error("AI is temporarily busy. Try again!")

# 2. VISUAL DASHBOARD (INFOGRAPHICS)
with info_tab:
    if raw_text and ai_model:
        st.markdown("### 📊 Interactive Visual Guide")
        with st.spinner("Generating Bento Infographics..."):
            try:
                # Optimized prompt for the Bento UI
                gen_p = f"Summarize {chapter} into 4 distinct bullet points: CORE CONCEPT, REAL LIFE USE, EXPERIMENT, BOARD TIP. Context: {raw_text[:4000]}"
                res_text = ai_model.generate_content(gen_p).text
                pts = res_text.split('*') # Split by bullet points
                ...
                with c_2:
                    ...
                    img = get_wiki_img_pro(term)
                    if img: st.image(img, use_container_width=True)
                    else: st.graphviz_chart("digraph { A[label='Concept'] -> B[label='Logic'] }")
...
with cheat_tab:
    if raw_text and ai_model:
        ...
        with st.spinner("Writing Flashcards..."):
            try:
                cheat_p = f"From {raw_text[:6000]}, list exactly 4 FORMULAS and 4 BOARD TIPS. Use tags FORMULA: and TIP:"
